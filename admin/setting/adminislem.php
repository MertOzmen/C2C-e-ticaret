...
if (isset($_POST['kullaniciresimguncelle'])) {
	...
	$uniqb = uniqid();
	$refimgyol = substr($uploads_dir, 6) . "/" . $uniqb . "." . $ext;

	@move_uploaded_file($tmp_name, "$uploads_dir/$uniqb.$ext");
...
if (isset($_POST['magazaresimguncelle'])) {

	if ($_FILES['kullanici_mazaresim']['size'] > 1048576) {

		echo "Bu dosya boyutu çok büyük";

		Header("Location:../../magaza-resim-guncelle.php?durum=dosyabuyuk");
	}

	$izinli_uzantilar = array('jpg', 'png');

	$ext = strtolower(substr($_FILES['kullanici_mazaresim']["name"], strpos($_FILES['kullanici_mazaresim']["name"], '.') + 1));

	if (in_array($ext, $izinli_uzantilar) === false) {
		echo "Bu uzantı kabul edilmiyor";
		Header("Location:../../magaza-resim-guncelle.php?durum=formathata");

		exit;
	}

	@$tmp_name = $_FILES['kullanici_mazaresim']["tmp_name"];
	@$name = $_FILES['kullanici_mazaresim']["name"];
	...
	$duzenle = $db->prepare("UPDATE kullanici SET
		kullanici_mazaresim=:kullanici_mazaresim
		WHERE kullanici_id={$_SESSION['userkullanici_id']}");
	$update = $duzenle->execute(array(
		'kullanici_mazaresim' => $refimgyol
	));
	...
	if ($update) {
		...
		Header("Location:../../magaza-resim-guncelle.php?durum=ok");
	} else {

		Header("Location:../../magaza-resim-guncelle.php?durum=hata");
	}
}
...
if (isset($_POST['magazaurunduzenle'])) {
	...
	if ($_FILES['urunfoto_resimyol']['size'] > 0) {
		...
		if ($update) {
			...
			Header("Location:../../urun-duznele.php?durum=hata&urun_id=$urun_id");
		}
	} else {
		//FOTO YOKSA İŞLEMLER
